...
<head>
    ...
    <script language="javascript" type="text/javascript">
            ...
            $.ajax({
                ...
                success:function (result) {
                    ...
                    $("#"+uuid+" .deletePic").click(function () {
                        ...
                        console.log("delete picName"+picName);
                        deletePhoto(picName);
                        $("#"+uuid).remove();
                    });
        ...
        function loadPic() {
            ...
            var data = $("form").serialize();
            $.ajax({
                url:"${ctx}/dev/contract/devContract/loadPic?contractId="+$("#contractId").val(),
                type: 'GET',
                cache: false,
                dataType: "json",
                success:function (result) {
                     for(var i=0;i<result.length;i++){
                         preview_picture_url(result[i]);
        ...
        //show url
        function preview_picture_url(pic) {

                var mydate = new Date();
                var uuid = "pic"+mydate.getDay()+ mydate.getHours()+ mydate.getMinutes()+mydate.getSeconds()+mydate.getMilliseconds();
                var html =" <div id="+uuid+" style=\"width: 100px;height: 150px;margin: 0px 20px;float: left\">\n" +
                    "        <span style=\"position: absolute;margin-left: 30px;margin-top: 120px; display: none\" class='deletePic' >删除</span>\n" +
                    "        <img src=\""+pic+"\" width=\"100px\" height=\"150px\"/>\n" +
                    "    </div>";
                $("#imgs").append(html).show();
                $("#uuid").text(uuid);
                $("#"+uuid+" img").click(function(){
                ...
                var subPicName = pic.substring(pic.lastIndexOf("/")+1,pic.length);
                $("#"+uuid+" .deletePic").append("<span style=\"display: none\">"+subPicName+"</span>");
                $("#"+uuid+" .deletePic").show();
                $("#"+uuid+" .deletePic").click(function () {
                    var picName = $("#"+uuid+" .deletePic span").text();
                    deletePhoto(picName);
...
<form enctype="multipart/form-data">
    ...
    <span></span>
    <div>
    <span id="uploadImg" style="margin-top: -40px;">

    <input type="file" id="picture" size="1" name="picture" >
    <a  id="addPic" href="#" >上传图片</a></span>
    </div>
    <span id="msg" style="margin-left: 60px;"></span>
    <input id="contractId" name="contractId" value="${contractId}" style="display: none">
</form>
<div id="imgs" style="margin-top: 50px;">
</div>
