...
public class SystemService extends BaseService implements InitializingBean {
    ...
    /**
     * create by chang 2020-06-23
     *
     * @param loginName
     * @param plaintPassword
     * @return
     */
    public boolean validPassword(String loginName, String plaintPassword) {
        User user = userDao.getByLoginName(new User(null, loginName));
        if (user != null) {
            return SystemService.validatePassword(plaintPassword, user.getPassword());
        }
        return false;
    }
    ...
    public void saveUser(User user) {
        //add 2020-06-29 by chang
        String level = user.getLevel();
        double coefficient;
        try {
            coefficient = Double.parseDouble(level) / 10;
        } catch (NumberFormatException e) {
            e.printStackTrace();
            coefficient = 0;
        }
        user.setCoefficient(coefficient);
        //add 2020-06-29 by chang
